...
class GeminiClient:
    ...
    async def get_conversational_decision(self, prompt: str) -> ConversationalResponse:
        max_retries = 3
        models_to_try = [self.interviewer_model_name, "gemini-2.5-flash-lite", "gemini-2.0-flash-lite", "gemini-1.5-flash", "gemini-2.5-flash", "gemini-1.5-pro"]
    ...
    async def extract_answer(self, prompt: str) -> ExtractionResponse:
        max_retries = 3
        models_to_try = [self.extractor_model_name, "gemini-2.5-flash-lite", "gemini-2.0-flash-lite", "gemini-1.5-flash", "gemini-2.5-flash", "gemini-1.5-pro"]
        unique_models = list(dict.fromkeys([m for m in models_to_try if m]))
